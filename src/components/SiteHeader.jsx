...
export default function SiteHeader() {
  ...
  return <header className={`nav-wrap ${menuOpen ? 'menu-open' : ''}`}>
    <nav className="nav container">
      <Link className="brand" href="/" aria-label="CodeASystem home"><img className="brand-mark" src={logoPath} width="44" height="44" alt="" decoding="async"/><span>CodeASystem</span></Link>
      <div className="nav-links">{sections.map(([, label, id]) => <SectionLink sectionId={id} key={id}>{label}</SectionLink>)}<Link href="/case-study">Case studies</Link></div>
